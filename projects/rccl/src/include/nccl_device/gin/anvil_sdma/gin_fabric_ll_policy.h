...
// Host-only helpers below need <cerrno>/<cstdlib>/<cstring>. They are included
// here, at file scope, rather than next to their users further down: those
// users sit inside namespace gin::fabric, and a consumer of the installed copy
// that reaches this header before libc would otherwise declare std, getenv,
// strtoull and memset inside gin::fabric.
#include <cerrno>
#include <cstddef>
#include <cstdint>
#include <cstdlib>
...
inline bool ginFabricA2ALaneTryBuild(GinFabricA2ACommState const& comm, bool ddaLLEnabled, size_t llThreshold,
  ...
  // llEpochDev here is only an eligibility probe (DDA allocated an epoch array).
  // Runtime GIN LL uses a private allocation of the same length.
