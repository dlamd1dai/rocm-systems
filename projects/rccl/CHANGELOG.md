...
* Added GPU-initiated device-API AllToAll on gfx1250 + MNNVL GIN-SDMA (`NCCL_GIN_TYPE=7`): one Test#5 kernel reads the Anvil GPU context and picks fabric DDA LL vs `gin.put` / SDMA from size/scratch/threshold. The LL lane also requires `RCCL_DDA_LL` (on by default). Size gate: `RCCL_GIN_FABRIC_LL_THRESHOLD_ALLTOALL` or `NCCL_GIN_FABRIC_LL_THRESHOLD_ALLTOALL` (unset defaults to 256 KiB; `0` disables the LL lane). Host query export: `ncclGinQueryFabricA2ALane` in installed `nccl_device/gin/gin_fabric_a2a_host.h`. The shared device body is `nccl_device/gin/anvil_sdma/gin_fabric_ll_a2a_device.h`.

### Changed
* GIN-SDMA device-API AllToAll fabric-LL occupancy defaults to 4 blocks-per-peer (`RCCL_GIN_FABRIC_LL_A2A_MAX_BPP`, max 8). The LL vs `gin.put`/SDMA size gate defaults to 256 KiB when `RCCL_GIN_FABRIC_LL_THRESHOLD_ALLTOALL` is unset (no longer follows `RCCL_DDA_LL_THRESHOLD`).
* `--rocshmem-gin` now requires the device linker. Combining it with `--no-device-linker` is a hard error (fat-linking extracted `librocshmem.a` hits duplicate `__hip_cuid_*`).
...
### Resolved issues
* Skip InfiniBand RMA proxy verbs init (`ibv_get_device_list`) when `NCCL_GIN_TYPE` is Anvil-SDMA (`6`), or when it is unset (`-1`) on gfx1250 only. A second verbs enumeration after NET/IB-CAST can SIGSEGV on some MI455 rdma-core stacks; GDAKI and non-gfx1250 unset GIN still initialize IB.
* GIN-SDMA fabric LSA AllToAll eligibility now refuses `nRanks` above 64, matching the LSA barrier CTA pool.
...
* GIN-SDMA fabric AllToAll setup now fails communicator create when the small-message lane allgather errors, instead of treating a bootstrap failure as "lane not eligible".
* GIN-SDMA fabric-LL AllToAll uses a GIN-private epoch buffer instead of sharing `comm->ddaLLEpochDev` with host DDA.
* GIN-SDMA fabric-LL AllToAll takes a device busy lock (`fabricA2ALlBusy`) so a second overlapping GIN LL launch on another stream fails the kernel instead of racing the epoch/scratch or mixing LL with `gin.put`.
