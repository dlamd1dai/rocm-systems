...
* Added GPU-initiated device-API AllToAll on gfx1250 + MNNVL GIN-SDMA (`NCCL_GIN_TYPE=6`): small messages invoke the shared fabric DDA LL body from a device kernel; large messages keep `gin.put` / SDMA. The LL lane also requires `RCCL_DDA_LL` (on by default). Size gate: `RCCL_GIN_FABRIC_LL_THRESHOLD_ALLTOALL` or `NCCL_GIN_FABRIC_LL_THRESHOLD_ALLTOALL` (unset falls back to `RCCL_DDA_LL_THRESHOLD`; `0` disables the LL lane). The shared device body is installed as `nccl_device/gin/anvil_sdma/gin_fabric_ll_a2a_device.h`.
