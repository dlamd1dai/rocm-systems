...
#          Hard-capped at 64M (host path faults above 64M on MI350/MI355).
...
HOST_MAX_BYTES="${HOST_MAX_BYTES:-64M}"
HOST_HARD_CAP_BYTES="${HOST_HARD_CAP_BYTES:-67108864}"  # 64M — host ncclAllGather hard limit
...
# AG-C1 sweep cap: min(HOST_MAX_BYTES, MAX_BYTES, 64M hard cap).
...
if [[ "${HOST_MAX_BYTES_EFFECTIVE_INT}" -gt "${HOST_HARD_CAP_BYTES}" ]]; then
  HOST_MAX_BYTES_EFFECTIVE_INT="${HOST_HARD_CAP_BYTES}"
fi
...
if [[ "${RUN_HOST_BASELINE}" != "0" ]]; then
  echo "  AG-C1 host:  ${MIN_BYTES} .. ${HOST_MAX_BYTES_EFFECTIVE} (hard cap 64M, NCCL_CUMEM_ENABLE=0)"
else
