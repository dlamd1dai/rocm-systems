...
DOCKER_IMAGE="${DOCKER_IMAGE:-rccl-gin-gda-sdma-ag}"
RCCL_GIN_RUN_TESTS="${RCCL_GIN_RUN_TESTS:-${RUN_TESTS:-0,3}}"
...
# Shared measurement iteration counts applied to BOTH tests so the host and GIN
# paths are compared over the SAME warmup(-w)/timed(-n) counts.
AG_WARMUP="${AG_WARMUP:-5}"
AG_ITERS="${AG_ITERS:-20}"

# GIN test data type/op for the AllGather (AllGather is op-less; -o none). Default
# int8 (float/half/bf16 all valid; AllGather is a pure copy so dtype only sets the
# element size / datacheck pattern).
AG_DTYPE="${AG_DTYPE:-int8}"

# gfx950 intermittent cuMem-VMM connectivity-gate abort retry budget (matches the
# A2A/AR harnesses): re-launch the GIN test up to this many times if the LSA
# signal connectivity gate fails at comm init (a genuine datacheck mismatch is
# NOT retried).
...
# --- Test#0: host-initiated AllGather baseline (all_gather_perf -D 0) ---
# Pure NCCL host path; no GIN. Pins the channel count so the tuner does not
# collapse channels on large messages (mirrors the A2A ring baseline).
if _run_test 0; then
  _trace_on
  TEST0_NCHANNELS="${TEST0_NCHANNELS:-32}"
  echo "=== Test#0: AllGather, ${NP} gpus, Host Initiated (channels=${TEST0_NCHANNELS}, ${MIN_BYTES}..${MAX_BYTES}) ==="
  ${DOCKER_CMD} run ${DOCKER_GPU} "${DOCKER_IMAGE}" \
    ...
    -x NCCL_GIN_ENABLE=0 \
    -x NCCL_GIN_TYPE=0 \
    -x NCCL_MIN_NCHANNELS="${TEST0_NCHANNELS}" \
    -x NCCL_MAX_NCHANNELS="${TEST0_NCHANNELS}" \
    rccl-tests/all_gather_perf -b "${MIN_BYTES}" -e "${MAX_BYTES}" -f 2 -g 1 -R 0 -D 0 -d "${AG_DTYPE}" -w "${AG_WARMUP}" -n "${AG_ITERS}"
  _trace_off
fi
...
# ctx->sdmaThreshold), so tuning AllGather no longer perturbs AllToAll or the
# backend. Default 32 KiB/rank is the measured LSA<->SDMA crossover on 8x MI355X
...
# all-SDMA puts, or a value >= the max per-rank chunk to force all-LSA (the
# per-collective knob is host-resolved as 64-bit, so large values do not wrap).
if _run_test 3; then
  _trace_on
  ...
  for _try in $(seq 1 "${GIN_CONN_RETRIES}"); do
    _ag_gin "${_ag_log}"; _ag_rc=$?
    # A genuine numeric datacheck mismatch must fail immediately (not retried).
    if grep -Eqi "Wrong|mismatch|check.*fail" "${_ag_log}" 2>/dev/null; then break; fi
    # Intermittent gfx950 cuMem-VMM connectivity-gate abort: re-launch.
    if [[ "${_ag_rc}" -ne 0 ]] && grep -Eqi "LSA signal connectivity gate failed|unhandled system error" "${_ag_log}" 2>/dev/null; then
      echo "=== Test#3: connectivity-gate abort (attempt ${_try}/${GIN_CONN_RETRIES}); re-launching after settle ===" >&2
      sleep 3
      continue
    fi
    break
