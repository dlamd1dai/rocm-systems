#!/usr/bin/env bash
# One-command verify for the GIN-SDMA SendRecv LL (low-latency) design on the SUT.
# Primary SUT: smci355-ccs-aus-m03-17 (8x MI355X, ~/rocm-systems, ~/rt-build).
#
# Runs, in order:
#   1. BUILD  sendrecv_perf + gin_sdma_policy_test (incremental, no GPU)
#   2. UT     gin_sdma_policy_test  (host-only policy tiering tests, no GPU)
#   3. GATE   gin_sdma_gpu_functional.sh sendrecv  (-c 1 correctness on NP GPUs;
#             exercises LL-default, GIN-forced, and LL-disabled passes)
#   4. AB     tiny-size LL-on vs LL-off latency sweep (NP GPUs)
#   5. BIG    (opt-in) forced-GIN -c 1 across the 1 GiB chunk boundary
#             (512M/1G/2G) -- proves the 30-bit SDMA copy-count fix (ginPutChunked);
#             1 GiB is exactly one max segment (count=2^30-1), 2 GiB is two
#             segments. The 2 GiB row previously reported #wrong=2^31 pre-fix.
#
# Every GPU container is given a unique --name and killed on EXIT, so a hung/
# timed-out run cannot leave zombie sendrecv_perf ranks holding GPU contexts
# (which otherwise cause the next run to fail with HIP 'invalid argument').
#
# Usage: ./gin-sdma-sendrecv-ll-verify.bash [NP]
#   NP  default 8
#
# Toggle steps:   RUN_BUILD=0 RUN_UT=0 RUN_GATE=0 RUN_AB=0 RUN_BIG=1
# Sizes/iters:    GATE_MAX_BYTES (default 1M), AB_MAX_BYTES (default 2048),
#                 AB_ITERS (default 50), AB_WARMUP (default 20)
#                 BIG_MIN_BYTES (default 512M), BIG_MAX_BYTES (default 2G)
...
# HW 30-bit copy-count max), the -D 3 GIN path is size-safe -- sweeps may exceed
# 1 GiB without the old copy-count truncation. RUN_BIG is opt-in only because
# 2 GiB x NP needs large HBM + longer wall time, not because it is unsafe.
...
AB_MIN_BYTES="${AB_MIN_BYTES:-8}"
AB_MAX_BYTES="${AB_MAX_BYTES:-2048}"
AB_ITERS="${AB_ITERS:-50}"
AB_WARMUP="${AB_WARMUP:-20}"
BIG_MIN_BYTES="${BIG_MIN_BYTES:-512M}"   # 512M single seg; 1G = exactly one max seg
BIG_MAX_BYTES="${BIG_MAX_BYTES:-2G}"     # 2G crosses the 1 GiB chunk edge (2 segs)
...
NUM_CHANNELS="${NUM_CHANNELS:-1}"            # keep 1 (>=2 deadlocks scatter GIN tier)
...
echo "=================================================================="
echo "SendRecv LL verify: NP=${NP} host=${_HOST} tag=${RUN_TAG}"
echo "  src=${SRC_HOST}  build=${RT_BUILD_HOST}  image=${DOCKER_IMAGE}"
echo "=================================================================="

# ---- 1. BUILD ----------------------------------------------------------------
if [[ "${RUN_BUILD}" != "0" ]]; then
  echo ">>> [1/4] BUILD sendrecv_perf + gin_sdma_policy_test"
  _cpu_run "${RUN_TAG}-build" "${BUILD_TIMEOUT}" -- \
    'cmake --build /rt-build --target sendrecv_perf gin_sdma_policy_test -j 8 2>&1 | grep -viE "ANVIL_ENABLED"'
  echo ">>> [1/4] BUILD ok"
fi

# ---- 2. POLICY UT ------------------------------------------------------------
if [[ "${RUN_UT}" != "0" ]]; then
  echo ">>> [2/4] POLICY UT (SendRecv + Move tiers)"
  _cpu_run "${RUN_TAG}-ut" "${UT_TIMEOUT}" -- \
    '/rt-build/gin_sdma_policy_test --gtest_filter="*SendRecv*:*Move*"'
  echo ">>> [2/4] POLICY UT ok"
fi

# ---- 3. FUNCTIONAL GATE (-c 1) ----------------------------------------------
if [[ "${RUN_GATE}" != "0" ]]; then
  echo ">>> [3/4] FUNCTIONAL GATE sendrecv (-c 1), max=${GATE_MAX_BYTES}"
  _gpu_run "${RUN_TAG}-gate" "${GATE_TIMEOUT}" -- \
    "GIN_SDMA_MAX_BYTES=${GATE_MAX_BYTES} GIN_SDMA_ITERS=${GATE_ITERS} GIN_SDMA_WARMUP=${GATE_WARMUP} \
     ...
     bash /src-tests/test/gin_sdma_gpu_functional.sh sendrecv /rt-build ${NP} mpirun"
...
if [[ "${RUN_AB}" != "0" ]]; then
  ...
  SR=/rt-build/sendrecv_perf
  PERF_ARGS="-b ${AB_MIN_BYTES} -e ${AB_MAX_BYTES} -f ${FACTOR} -g 1 -R ${GIN_RANKS} -V ${DEVICE_CTA_COUNT} -D 3 -c 1 -n ${AB_ITERS} -w ${AB_WARMUP} -z 0"
  _gpu_run "${RUN_TAG}-ab" "${AB_TIMEOUT}" -- "
    echo '==== SendRecv LL ON (default 2 KiB cutover) ===='
    mpirun -n ${NP} ${MPI_OPT} ${MPI_BASE[*]} ${SR} ${PERF_ARGS}
    echo
    echo '==== SendRecv LL OFF (NCCL_GIN_ANVIL_SENDRECV_LL_MAX_BYTES=0) ===='
    mpirun -n ${NP} ${MPI_OPT} ${MPI_BASE[*]} -x NCCL_GIN_ANVIL_SENDRECV_LL_MAX_BYTES=0 ${SR} ${PERF_ARGS}
  "
  echo ">>> [4/4] LL A/B ok  (compare the 'time' (us) column: LL should be lower at <=2 KiB)"
fi

# ---- 5. BIG chunk-boundary regression (opt-in) ------------------------------
# Forces the GIN tier (THRESHOLD_SENDRECV=0) at sizes that cross the 1 GiB
# ginPutChunked segment edge, so the previously-corrupting 2 GiB message is
# validated end-to-end (-c 1). Expect "# Out of bounds values : 0 OK" on every
# row; before chunking the 2 GiB row reported #wrong=2147483648 (2^31).
if [[ "${RUN_BIG}" != "0" ]]; then
  echo ">>> [5/5] BIG forced-GIN regression ${BIG_MIN_BYTES}..${BIG_MAX_BYTES} (-c 1)"
  SR=/rt-build/sendrecv_perf
  BIG_ARGS="-b ${BIG_MIN_BYTES} -e ${BIG_MAX_BYTES} -f ${FACTOR} -g 1 -R ${GIN_RANKS} -V ${DEVICE_CTA_COUNT} -D 3 -c 1 -n ${BIG_ITERS} -w ${BIG_WARMUP} -z 0"
  _gpu_run "${RUN_TAG}-big" "${BIG_TIMEOUT}" -- "
    echo '==== SendRecv forced-GIN, >=1 GiB (ginPutChunked 1 GiB segments) ===='
    mpirun -n ${NP} ${MPI_OPT} ${MPI_BASE[*]} -x NCCL_GIN_ANVIL_SDMA_THRESHOLD_SENDRECV=0 ${SR} ${BIG_ARGS}
  "
  echo ">>> [5/5] BIG ok  (every row must show '# Out of bounds values : 0 OK')"
fi

echo "=================================================================="
echo "PASS: gin-sdma-sendrecv-ll-verify  NP=${NP}  steps: build=${RUN_BUILD} ut=${RUN_UT} gate=${RUN_GATE} ab=${RUN_AB} big=${RUN_BIG}"
echo "=================================================================="
