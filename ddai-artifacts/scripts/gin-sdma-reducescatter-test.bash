...
# gin-sdma-scatter-test.bash / gin-sdma-gather-test.bash; ReduceScatter is the
# first REDUCTION collective, so it adds an op dimension (-o) over the two-way
# host-vs-GIN comparison.
#
...
#   RS-C2  GIN Anvil-SDMA ReduceScatter (-D 3, NCCL_GIN_TYPE=6, -V 32): each rank
#          reduces the contributions to its owned slice. Size-hybrid:
#            * slice <= threshold: LSA read-reduce (balanced pull, no scratch);
#            * slice >  threshold: put-partials into peer scratch + SM reduce.
#          Default cutover 256 KiB/rank slice (provisional; retune per the design
#          plan). Override with THRESHOLD (bytes/rank-slice; 0 = all-GIN).
#
...
# Device (-D 3) knobs, tuned 2026-07-28 on 8x MI355X (see sibling scripts):
#   DEVICE_CTA_COUNT (-V): 32 is near-optimal.
...
THRESHOLD="${THRESHOLD:-}"      # kernel LSA<->GIN cutover (bytes/rank-slice); empty = tuned 256 KiB
...
if [[ "${RUN_GIN_SDMA}" != "0" ]]; then
  echo "RS-C2: GIN ReduceScatter -D 3, ${MIN_BYTES}..${MAX_BYTES} (-R ${GIN_RANKS}, -V ${DEVICE_CTA_COUNT}, op ${OP}, threshold=${THRESHOLD:-256K default})"
