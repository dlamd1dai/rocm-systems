...
# Shared GIN host init for GDA + Anvil plugins (refactored from gin_host_rocshmem_common).
_preflight_require_file projects/rccl/src/include/gin/gin_host_rocshmem_gda.h

# Legacy common split; must not remain as stale stubs in the tree.
for stale in \
  projects/rccl/src/include/gin/gin_host_rocshmem_common.h \
  projects/rccl/src/gin/gin_host_rocshmem_common.cc; do
  if [[ -f "${stale}" ]]; then
    echo "ERROR: stale file should be removed (use gin_host_rocshmem_gda.h): ${stale}" >&2
    _preflight_fail=1
  fi
done
