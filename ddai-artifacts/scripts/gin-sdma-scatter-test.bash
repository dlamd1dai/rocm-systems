#!/usr/bin/env bash
# GIN Anvil SDMA Scatter gate + perf (docker or bare-metal).
...
#
# Usage: ./gin-sdma-scatter-test.bash [NP] [MAX_BYTES]
#   NP         default 8
#   MAX_BYTES  default 128M   (total send size; per-rank chunk = size/NP)
#
# Checks (order: SC-C1 host baseline -> SC-C2 GIN):
#   SC-C1  Host-initiated ncclScatter               (scatter_perf -D 0)
#   SC-C2  GIN Anvil-SDMA Scatter (-D 3, NCCL_GIN_TYPE=6, -V 32): the root
#          distributes per-rank chunk r to rank r. Size-hybrid LSA(small)/
#          GIN-SDMA(large): LSA is root-egress-bound (only the root stores), so
#          tuning (2026-07-27, 8x MI355X) found GIN/SDMA wins for chunks
#          >=256 KiB (default cutover 128 KiB; 512 MiB total: 390 vs 64 GB/s).
#          Override the cutover with THRESHOLD (bytes/rank-chunk; 0 = all-GIN).
#
# Full gate (default): SC-C1 + SC-C2.
...
FACTOR="${FACTOR:-2}"
# Host baseline channel pin. Scatter is P2P (grouped send from root); the stock
# tuner collapses channels and cliffs at mid/large sizes (8x MI355X, 2026-07-27:
# 128M 136->384 GB/s, no small-size loss). Pinning MIN=MAX lifts it uniformly;
# CE/SDMA (-R 2 CTA_POLICY=ZERO) is actually slower for P2P here, so it is not used.
HOST_NCHANNELS="${HOST_NCHANNELS:-32}"
ROOT="${ROOT:-0}"               # scatter root: integer, or 'all' to sweep 0..NP-1
THRESHOLD="${THRESHOLD:-}"      # kernel LSA<->GIN cutover (bytes/rank-chunk); empty = tuned 128 KiB

_HOST="$(hostname -s 2>/dev/null || hostname)"
SCATTER_PERF="${SCATTER_PERF:-rccl-tests/scatter_perf}"
...
echo "Scatter gate: NP=${NP} host=${_HOST} root=${ROOT}"
_gpu_reset_before_gate
...
SC_C1_STATUS="skipped"
# --- SC-C1: host-initiated ncclScatter (-D 0, no GIN) ---
if [[ "${RUN_HOST_BASELINE}" != "0" ]]; then
  echo "SC-C1: host Scatter -D 0, ${MIN_BYTES}..${MAX_BYTES} (-R ${HOST_RANKS}, nch ${HOST_NCHANNELS}, root ${ROOT})"
  _run mpirun -n "${NP}" ${MPI_OPT_RCCL} "${MPI_BASE_HOST[@]}" \
    -x NCCL_MIN_NCHANNELS="${HOST_NCHANNELS}" -x NCCL_MAX_NCHANNELS="${HOST_NCHANNELS}" \
    "${SCATTER_PERF}" -b "${MIN_BYTES}" -e "${MAX_BYTES}" -f "${FACTOR}" -g 1 -R "${HOST_RANKS}" -D 0 "${ROOT_ARG[@]}"
  SC_C1_STATUS="passed"
  sleep "${TEST_GAP_SEC:-3}"
fi

# --- SC-C2: GIN Anvil-SDMA Scatter kernel (-D 3, NCCL_GIN_TYPE=6) ---
if [[ "${RUN_GIN_SDMA}" != "0" ]]; then
  echo "SC-C2: GIN Scatter -D 3, ${MIN_BYTES}..${MAX_BYTES} (-R ${GIN_RANKS}, -V ${DEVICE_CTA_COUNT}, root ${ROOT}, threshold=${THRESHOLD:-128K default})"
  _run mpirun -n "${NP}" ${MPI_OPT_RCCL} "${MPI_BASE[@]}" \
    ...
    ${THRESHOLD:+-x NCCL_GIN_ANVIL_SDMA_THRESHOLD_SCATTER="${THRESHOLD}"} \
    -x HSA_FORCE_FINE_GRAIN_PCIE=1 \
    "${SCATTER_PERF}" -b "${MIN_BYTES}" -e "${MAX_BYTES}" -f "${FACTOR}" -g 1 -R "${GIN_RANKS}" -V "${DEVICE_CTA_COUNT}" -D 3 "${ROOT_ARG[@]}"
  sleep "${TEST_GAP_SEC:-3}"
fi

echo "PASS: gin-sdma-scatter-test np=${NP} root=${ROOT} SC-C1=${SC_C1_STATUS} SC-C2=${MIN_BYTES}..${MAX_BYTES}"
