...
if [[ $# -lt 2 ]]; then
  ...
  echo "  $0 ... true   # or sudo docker build --build-arg ROCSHMEM_CACHE_BUST=N ..."
  ...
  echo "Test#4 default: GIN_ANVIL (-D 3, NCCL_GIN_TYPE=5). Host IB proxy (-D 3, TYPE=2): export RUN_GIN_HOST_PROXY=1 (needs working IB + ncclGinIb)."
...
# docker build: BuildKit attaches RUN steps to the default bridge. Some hosts have no docker0
# ("network bridge not found"). --network=host avoids that. Override: DOCKER_BUILD_NETWORK=default
# when the bridge exists and you want isolated build networking.
DOCKER_BUILD_NETWORK="${DOCKER_BUILD_NETWORK:-host}"

# --- build
if ${BUILD_FLAG}; then
  N=1
  sudo docker build --network="${DOCKER_BUILD_NETWORK}" -f ${DOCKERFILE} -t ${DOCKER_IMAGE} \
    --no-cache \
...
# --- sanity: image layout
sudo docker run --rm ${DOCKER_IMAGE} bash -lc "
  echo '=== workspace ==='
  pwd
...
set -x
sudo docker run ${DOCKER_GPU} ${DOCKER_ROCSHMEM_EXTRA} ${DOCKER_IMAGE} \
  mpirun ${MPIRUN_BASE} \
...
if [ 1 -eq 1 ]; then
#####
# Test#4: GinAlltoAllKernel (-D 3). NCCL_GIN_TYPE=2 (host IB proxy) frequently hits ncclSystemError at
# ncclDevCommCreate when IB/ncclGinIb is not fully usable (still common with NCCL_GIN_PLUGIN=none on lab nodes).
# Default matches Test#8: GIN_ANVIL (TYPE=5) single-node smoke. Opt in to TYPE=2: RUN_GIN_HOST_PROXY=1.
