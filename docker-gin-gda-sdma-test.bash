...
TEST5_NUM_CTAS="${TEST5_NUM_CTAS:-16}"
...
if _should_run_test5; then
  _trace_on
  echo "=== Test#5: A2A, ${NP} gpus, GIN Anvil SDMA (NCCL_GIN_TYPE=5); -D 3 GinAlltoAll (-V ${TEST5_NUM_CTAS}, LSA small + GIN/SDMA large) ==="
  TEST5_MPI_EXTRA=()
  ...
  if [[ -n "${NCCL_GIN_ANVIL_SDMA_FUSED_SIGNAL:-}" ]]; then
    TEST5_MPI_EXTRA+=(-x "NCCL_GIN_ANVIL_SDMA_FUSED_SIGNAL=${NCCL_GIN_ANVIL_SDMA_FUSED_SIGNAL}")
  fi
  if [[ -n "${NCCL_GIN_ANVIL_SDMA_OSS7:-}" ]]; then
    TEST5_MPI_EXTRA+=(-x "NCCL_GIN_ANVIL_SDMA_OSS7=${NCCL_GIN_ANVIL_SDMA_OSS7}")
  fi
  if [[ -n "${RCCL_GIN_ALLTOALL_BATCHED:-}" ]]; then
    TEST5_MPI_EXTRA+=(-x "RCCL_GIN_ALLTOALL_BATCHED=${RCCL_GIN_ALLTOALL_BATCHED}")
  fi
  ...
  if [[ "${RCCL_GIN_ALLTOALL_HOST_TRACE:-}" == 1 ]]; then
    TEST5_MPI_EXTRA+=(-x "RCCL_GIN_ALLTOALL_HOST_TRACE=1")
  fi
  if [[ "${RCCL_GIN_ALLTOALL_DEVICE_TRACE:-}" == 1 ]]; then
    TEST5_MPI_EXTRA+=(-x "RCCL_GIN_ALLTOALL_DEVICE_TRACE=1")
  fi
  ${DOCKER_CMD} run ${DOCKER_GPU} ${D_MEMLOCK[*]} ${DOCKER_TEST5_MLX5_VOLUMES} "${DOCKER_IMAGE}" \
    ...
    -x HSA_FORCE_FINE_GRAIN_PCIE=1 \
    "${TEST5_MPI_EXTRA[@]}" \
    rccl-tests/alltoall_perf -b "${MIN_BYTES}" -e "${MAX_BYTES}" -f 2 -g 1 -R 2 -D 3 -A 1 -V "${TEST5_NUM_CTAS}"
  _trace_off
fi
